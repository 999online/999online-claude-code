#!/usr/bin/env node
// folder-structure-guard.cjs -- PreToolUse hook that enforces the project's
// folder contract declared in .claude/allowed-paths.json.
//
// Three possible decisions:
//   - deny (hard block): path matches a forbiddenGlobs entry. Exit 0 with
...
// `?` (single char). Good enough for the allowlist; not a full fnmatch.
  ...
  while (i < glob.length) {
    ...
    if (c === '*') {
      ...
      if (end === -1) {
        ...
        const parts = glob.slice(i + 1, end).split(',').map(s => s.trim());
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    function emitDeny(reason) {
      const payload = {
        // Legacy top-level fields for older Claude Code versions that check `decision`.
        decision: 'block',
        reason,
        hookSpecificOutput: {
    ...
    for (const g of forbiddenGlobs) {
      if (globToRegex(g).test(relPath)) {
        emitDeny(
          'BLOCKED: ' + relPath + ' matches forbidden pattern "' + g +
          '" from .claude/allowed-paths.json. This pattern is explicitly disallowed ' +
          'by CLAUDE.md "Project Structure (STRICT)" and cannot be bypassed. ' +
    ...
    if (!relPath.includes('/') && relPath.endsWith('.md')) {
      ...
      emitDeny(
        'BLOCKED: "' + relPath + '" is a Markdown file at the project root. ' +
        'All .md files must be created inside docs/ (e.g. "docs/' + relPath + '"). ' +
        'Only these are allowed at root: ' + exemptRootMd.join(', ') + '. ' +
    ...
    if (!relPath.includes('/')) {
      ...
      emitAsk(
        'Creating new root-level file "' + relPath + '" is outside the allowlist. ' +
    ...
    emitAsk(
      'Creating new file "' + relPath + '" is outside the folder contract in .claude/allowed-paths.json. ' +
