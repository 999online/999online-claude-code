...
- No junk-drawer dirs: `utils/`, `helpers/`, `common/`, `shared/`, `misc/`. Shared code has a named home (`lib/`, `services/`).
- No new top-level dirs without updating contract.
- Root `.md` files: only README.md and CLAUDE.md. Everything else → `docs/`.
- Business logic never in route handlers/pages — thin handlers, logic in services/lib.
