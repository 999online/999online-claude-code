...
2. **Scaffold.**
   ...
   - **Monorepo (path B, C):** scaffold each app into its subdir — `apps/web` (path frontend scaffolder), `apps/api` (Hono — always for path C; path B unless the ADR collapses it for a single-surface app). Then author the workspace glue per current Turborepo docs (context7/WebSearch): root `package.json` (`"workspaces": ["apps/*","packages/*"]`, `turbo@latest` dev dep), `turbo.json`, `tsconfig.base.json` (path aliases), `.prettierrc.json`, `eslint.config.mjs`, `.nvmrc`. Stub `packages/db`: `drizzle.config.ts` (dialect sqlite/D1), `src/index.ts` (`createDb(d1)` client), `src/schema.ts` (empty/example schema — no real entities), `migrations/.gitkeep`. Inspect the assembled tree before step 7.
