...
2. **Scaffold.**
   ...
   - **Monorepo (path B, C):** scaffold each app into its subdir — `apps/web` (path frontend scaffolder), `apps/api` (Hono — **always present for both B and C; never collapse**). Then author the workspace glue per current Turborepo docs (context7/WebSearch): root `package.json` (`"workspaces": ["apps/*","packages/*"]`, `turbo@latest` dev dep), `turbo.json`, `tsconfig.base.json` (path aliases), `.prettierrc.json`, `eslint.config.mjs`, `.nvmrc`. Stub `packages/db`: `drizzle.config.ts` (dialect sqlite/D1), `src/index.ts` (`createDb(d1)` client), `src/schema.ts` (empty/example schema — no real entities), `migrations/.gitkeep`.
     - **Consistent workspace naming:** set package names `@app/web`, `@app/api`, `@app/db` (scaffolders name packages after the dir — rename), and make root scripts reference workspaces as `-w @app/web` etc.
     - **tsconfig wiring:** edit each app's `tsconfig.json` to `"extends": "../../tsconfig.base.json"` and re-declare `@/*` → `./src/*` plus the `@app/db` alias at its own depth. Never leave the scaffolder's standalone tsconfig unlinked from the base.
     - Inspect the assembled tree before step 7.
