...
Layout: B/C are always the **three-workspace monorepo** (`apps/web` + `apps/api` + `packages/db`, Turborepo) — `apps/api` (Hono) is always present for both B and C; never collapse it into Next route handlers. A/E stay single-package. Cloudflare is the house default deploy target (Workers/Pages/D1/R2/KV/Access); record the ADR `Layout` field.
